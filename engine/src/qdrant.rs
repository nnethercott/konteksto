...
use qdrant_client::{
    ...
    qdrant::{
        ...
        QueryPointsBuilder, Sample, ScoredPoint, ScrollPointsBuilder, UpsertPointsBuilder, Vector,
        VectorParamsBuilder, Vectors, vectors_output::VectorsOptions,
    },
};
...
impl Into<PointStruct> for Entry {
    fn into(self) -> PointStruct {
        let payload: Payload = json!({"word": self.content}).try_into().unwrap();
...
fn get_inner_vec(v: &ScoredPoint) -> Option<Vec<f32>> {
...
impl Client {
    ...
    pub async fn create_from_dump(&self, file: &str, collection_name: &str) -> Result<()> {
        ...
        self.create_collection(
            CreateCollectionBuilder::new(collection_name).vectors_config(VectorParamsBuilder::new(
                entries[0].embedding.len() as u64,
                Distance::Euclid,
            )),
        )
        .await?;
    ...
    pub async fn get_random_query(
        ...
        let mut vectors: Vec<Vec<f32>> = res.result.iter().filter_map(|v| get_inner_vec(v)).collect();
